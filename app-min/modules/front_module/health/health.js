...
angular.module('myApp.health', ['ui.router'])


    .controller('HealthCtrl', ['$rootScope','$scope','locals','$state','$stateParams','$http','$compile','$location','$anchorScroll',function($rootScope,$scope,locals,$state,$stateParams,$http,$compile,$location,$anchorScroll) {
        //加载延迟设定
        var waitTime=5;
        var stateName=locals.getObj('stateName',1000*3600*24);
        if(locals.get('upPost')){//访问的博客路径name改变时，需延迟加载给headBar提供请求时间
            waitTime=800;
        }
        var run=setTimeout(function() {
            if(angular.isArray(locals.get('healthPosts',1000*3600*24*3))){
                var posts=locals.get('healthPosts',1000*3600*24*3);
                console.log(posts)
                var countPost=20;
                ...
                if(posts[r].postId=='1513001280665'){
            ...
            $http({url:'http://localhost:3000/label_posts',
                ...
                locals.setObj('healthPosts',posts);
                var countPost=20;
                $scope.noMore_show=false;
                $scope.load_show=true;
                var ul1=angular.element(document.querySelectorAll('#artUl1'));
