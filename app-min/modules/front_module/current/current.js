...
angular.module('myApp.current', ['ui.router'])


    .controller('CurrentCtrl', ['$rootScope','$scope','locals','$state','$stateParams','$http','$compile','$location','$anchorScroll',function($rootScope,$scope,locals,$state,$stateParams,$http,$compile,$location,$anchorScroll) {

            if(locals.getObj('currentPosts',1000*3600*24*3)){
                var posts=locals.getObj('currentPosts',1000*3600*24*3);
                var countPost=20;
            ...
            $http({url:'http://localhost:3000/label_posts',
                ...
                locals.setObj('currentPosts',posts);
                var countPost=20;
                $scope.noMore_show=false;
                $scope.load_show=true;
                var ul1=angular.element(document.querySelectorAll('#artUl1'));
