...
app.controller('index',['$http','locals',function($http,locals){
            ...
            var run=$timeout(function(){
                ...
                var getUserSession=function(){
                    ...
                    if($scope.loginUser!=undefined){
                        ...
                        var html="<span ui-sref='blog({name:&#39;{{userName}}&#39;})'  ng-show='blogBtn_show'  style='background:darkred;padding:10px 5px;color:#c4c4c4;font-weight:bold;margin-left:10px;cursor:pointer;' >进入博客</span>";
                        ...
                        var text="<a style='margin-left:20px;'>欢迎回来!</a><a  style='text-decoration:underline;'>"+user.name+"</a>";
                        userLogin.html(text);
                    }else{
                        text="<a>您好,请登录!</a>";
                        $scope.loginBtn_Show=true;
                ...
                if(locals.getObj('lastName',1000*3600*24*7)){
                    ...
                    $scope.loginBtn_Show=false;
                    $scope.blogBtn_show=true;
                    var text="<a style='margin-left:20px;'>欢迎回来!</a><a  style='text-decoration:underline;'>"+name+"</a>";
                    userLogin.html(text);
                    //添加进入博客button
                    var html="<span ui-sref='blog({name:&#39;"+name+"&#39;})'  ng-show='blogBtn_show'  style='background:darkred;padding:10px 5px;color:#c4c4c4;font-weight:bold;margin-left:10px;cursor:pointer;' >进入博客</span>";
                    html=$compile(html)($scope);
                    blogBtn.empty();
                    blogBtn.append(html);
                }else{
                ...
                $scope.login=function(){
                    var name=filterXSS($scope.user_name);
                    var pwd=filterXSS($scope.user_password);
                    var date=new Date();
                    console.log(pwd+' '+name)
                    var lastLogin=date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate() + " " +
                        date.getHours() + ":" + (date.getMinutes() < 10 ? '0' + date.getMinutes() : date.getMinutes());
                    $http({url:"http://www.yblog.site:3000/login",
                ...
                /* var notActive=setTimeout(function(){   //访问删除七天未激活用户接口
                 $http({url:"http://www.yblog.site:3000/not_active",
                 method:'GET',
                 withCredentials: true
                 }).then(function(result){
                ...
                $scope.logout=function(){
                    $http({url:"http://www.yblog.site:3000/logout",
                        method:'GET',
                        withCredentials: true
                    }).then(function(result){
            ...
            var run=$timeout(function(){
                ...
                if(locals.getObj('lastName',1000*3600*24*7)==$stateParams.name){
                    ...
                    if(locals.getObj('lastName',1000*3600*24*7)){
                        ...
                        var text3='游客，<a ui-sref="home.headline" target="_blank" style="cursor:pointer;color:blue;font-size:12px;text-decoration:underline;">去登录</a>';
                ...
                if (!$scope.loadedData) {
                    ...
                    for (var i = 0; i < postTypes.length; i++) {
                        if (postTypes[i]) {
                            ...
                            type = type[1];
                            var html = '<p ng-click="type' + i + '_click()"><span><img style="width:15px;height:15px;margin-top:-5px;" src="images/mytype.png"/></span><a href="">' + type + '(' + posts[i].length + ')</a></p>'
                            ...
                            for (var j = 0; j < posts[i].length; j++) {
                                ...
                                $scope.name = $stateParams.name;
                                var title = '<li class="post_title"  style="margin-top;3px;margin-bottom:3px;"><span><img style="width:15px;height:15px;margin-top:-5px;" src="images/article.png"/></span><a name="{{id}}" ui-sref="logDetail({postId:&#39;{{id}}&#39;,name:&#39;{{name}}&#39;})">' + posts[i][j].title + '</a></li>'; //
            ...
            var run=function(){
                if (!$scope.loadedData) {
                    ...
                    $scope.removeFocus = function () {
                        ...
                        $http({
                            url: 'http://www.yblog.site:3000/up_focus',
                            method: 'POST',
                    ...
                    $scope.removeFriend = function () {
                        ...
                        console.log('cookie' + token)
                        $http({
                            url: 'http://www.yblog.site:3000/up_friend',
                            method: 'POST',
                            data: {
                                myName: name,
                    ...
                    $scope.addFriend = function () {
                        ...
                        if (!locals.getObj('lastName', 1000 * 3600 * 24)) {
                            ...
                            console.log('cookie' + token)
                            $http({
                                url: 'http://www.yblog.site:3000/send_message',
                                method: 'POST',
